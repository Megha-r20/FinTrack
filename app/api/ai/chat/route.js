...
5. Educational advice only — never suggest trading or stocks.`;
                ...
                const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("Gemini AI API timeout")), 2500));
            ...
            if (queryLower.includes("daily") ||
                ...
                if (budgetAnalysis.length === 0) {
                    ...
                    aiResponseText = `Here is your **Hostel Category Budget Status**:\n\n` +
                        budgetAnalysis
                            .map((b) => `- **${b.category}**: ${user.currency}${b.spent.toLocaleString()} / ${user.currency}${b.limit.toLocaleString()} (**${b.percentage}% used**)${b.isExceeded ? " 🚨 *EXCEEDED*" : b.isWarning ? " ⚠️ *NEAR LIMIT*" : " ✅ *ON TRACK*"}`)
                            .join("\n") +
