    ...
    return (<div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleExportCSV} className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl text-xs font-extrabold bg-[#FFFFFF] dark:bg-[#201A1A] border border-[#E2DBD0] dark:border-[#3B3030] text-[#141010] dark:text-[#FAF8F5] hover:bg-[#FAF8F5] dark:hover:bg-[#302929] transition-colors shadow-sm">
      ...
      <div className="p-4 rounded-3xl bg-[#FFFFFF] dark:bg-[#201A1A] border border-[#E2DBD0] dark:border-[#3B3030] shadow-sm space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          <div className="lg:col-span-2 relative">
            ...
            <input type="text" placeholder="Search description, notes, category..." value={search} onChange={(e) => {
      ...
      <div className="bg-[#FFFFFF] dark:bg-[#201A1A] border border-[#E2DBD0] dark:border-[#3B3030] rounded-3xl shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-[#E2DBD0]/50 dark:divide-[#3B3030] font-medium">
              {loading ? (Array.from({ length: 5 }).map((_, idx) => (<tr key={idx} className="animate-pulse">
                  ...
                  <td colSpan={6} className="py-12 text-center text-[#594D4D]">
                ...
                </tr>) : (transactions.map((tx) => (<tr key={tx.id} className="hover:bg-[#FAF8F5] dark:hover:bg-[#302929] transition-colors">
                    <td className="py-4 px-5">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-xl flex items-center justify-center text-white shrink-0 shadow-sm" style={{ backgroundColor: tx.category?.color || "#810100" }}>
                    ...
                    <td className={`py-4 px-5 text-right font-black text-sm ${tx.type === "INCOME" ? "text-emerald-700 dark:text-emerald-400" : "text-[#810100] dark:text-[#FAF8F5]"}`}>
                      {tx.type === "INCOME" ? "+" : "-"}
                      {currency}
                      {tx.amount.toLocaleString()}
                    </td>
                    <td className="py-4 px-5 text-center">
                      <button onClick={() => handleDelete(tx.id)} className="p-1.5 text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/40 rounded-lg transition-colors" title="Delete">
        ...
        <div className="px-5 py-3.5 border-t border-[#E2DBD0] dark:border-[#3B3030] flex items-center justify-between text-xs font-bold text-[#594D4D]">
          ...
          <div className="flex items-center gap-2">
            <button disabled={page <= 1} onClick={() => setPage((p) => Math.max(1, p - 1))} className="p-2 rounded-xl border border-[#E2DBD0] dark:border-[#3B3030] disabled:opacity-40 hover:bg-[#FAF8F5] dark:hover:bg-[#302929]">
              <ChevronLeft className="w-4 h-4"/>
            </button>
            <button disabled={page >= pagination.totalPages} onClick={() => setPage((p) => p + 1)} className="p-2 rounded-xl border border-[#E2DBD0] dark:border-[#3B3030] disabled:opacity-40 hover:bg-[#FAF8F5] dark:hover:bg-[#302929]">
      ...
      {isImportModalOpen && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-xl bg-[#FFFFFF] dark:bg-[#201A1A] rounded-3xl shadow-2xl border border-[#E2DBD0] dark:border-[#3B3030] overflow-hidden">
            ...
            <div className="p-5 space-y-3 max-h-[60vh] overflow-y-auto">
              ...
              {importErrors.length > 0 && (<div className="p-3 bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900 rounded-xl space-y-1">
                  <div className="flex items-center gap-1.5 text-xs font-bold text-rose-600 dark:text-rose-400">
                    <AlertCircle className="w-4 h-4"/>
                    <span>Row Validation Warnings</span>
                  </div>
                  {importErrors.map((err, idx) => (<div key={idx} className="text-[11px] text-rose-500">
            ...
            <div className="p-4 border-t border-[#E2DBD0] dark:border-[#3B3030] flex items-center justify-end gap-2">
              <button onClick={() => setIsImportModalOpen(false)} className="px-4 py-2 rounded-xl text-xs font-semibold text-[#594D4D] hover:bg-[#FAF8F5] dark:hover:bg-[#302929]">
                Cancel
              </button>
              <button onClick={executeImport} disabled={importing} className="px-4 py-2.5 rounded-xl text-xs font-extrabold bg-gradient-to-r from-[#810100] to-[#630000] text-white shadow-md cherry-glow">
