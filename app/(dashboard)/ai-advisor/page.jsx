...
export default function AiAdvisorPage() {
    ...
    return (<div className="h-[calc(100vh-140px)] flex flex-col space-y-4 animate-in fade-in duration-300">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 shrink-0">
        ...
        {messages.length > 0 && (<button onClick={handleClearChat} className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-bold text-[#810100] dark:text-[#FAF8F5] hover:bg-[#810100]/10 border border-[#810100]/30 transition-colors self-start md:self-auto">
      ...
      <div className="flex-1 bg-[#FFFFFF] dark:bg-[#201A1A] border border-[#E2DBD0] dark:border-[#3B3030] rounded-3xl shadow-sm flex flex-col min-h-0 overflow-hidden">
        <div className="flex-1 p-4 md:p-6 overflow-y-auto space-y-4">
          {messages.length === 0 ? (<div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-4">
              ...
              <div className="flex flex-wrap items-center justify-center gap-2 max-w-xl pt-2">
                {PRESET_QUESTIONS.map((q, idx) => (<button key={idx} onClick={() => handleSend(q)} className="px-3.5 py-2.5 rounded-2xl text-xs font-bold bg-[#FAF8F5] hover:bg-[#E2DBD0]/60 dark:bg-[#141010] dark:hover:bg-[#302929] text-[#141010] dark:text-[#FAF8F5] border border-[#E2DBD0] dark:border-[#3B3030] transition-all text-left shadow-sm">
                    "{q}"
                  </button>))}
              </div>
            </div>) : (messages.map((msg, idx) => (<div key={idx} className={`flex gap-3 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                {msg.role !== "user" && (<div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-[#810100] to-[#630000] text-white flex items-center justify-center shrink-0 text-xs font-bold shadow-sm">
                    <Bot className="w-4 h-4"/>
                  </div>)}

                <div className={`max-w-[85%] md:max-w-[75%] p-4 rounded-2xl text-xs md:text-sm font-medium leading-relaxed whitespace-pre-line ${msg.role === "user"
                ...
                {msg.role === "user" && (<div className="w-8 h-8 rounded-xl bg-[#810100] text-white flex items-center justify-center shrink-0 text-xs font-bold shadow-sm">
                    {user?.name ? user.name.charAt(0).toUpperCase() : "M"}
                  </div>)}
              </div>)))}

          {sending && (<div className="flex gap-3 justify-start">
        ...
        <div className="p-4 border-t border-[#E2DBD0] dark:border-[#3B3030] bg-[#FAF8F5] dark:bg-[#141010]/60">
          <form onSubmit={(e) => {
            ...
            <input type="text" placeholder="Ask AI Advisor about your income, spending habits, or budgets..." value={inputMessage} onChange={(e) => setInputMessage(e.target.value)} className="flex-1 px-4 py-3 bg-[#FFFFFF] dark:bg-[#201A1A] border border-[#E2DBD0] dark:border-[#3B3030] rounded-2xl text-xs md:text-sm font-semibold text-[#141010] dark:text-[#FAF8F5] focus:outline-none"/>
            <button type="submit" disabled={sending || !inputMessage.trim()} className="p-3.5 rounded-2xl bg-gradient-to-r from-[#810100] to-[#630000] disabled:opacity-40 text-white shadow-md cherry-glow transition-all">
