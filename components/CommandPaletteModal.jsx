...
export function CommandPaletteModal({ isOpen, onClose, onOpenAddModal }) {
    ...
    const filteredTx = transactions.filter((tx) => tx.description.toLowerCase().includes(query.toLowerCase()) ||
    ...
    return (<div className="fixed inset-0 z-50 flex items-start justify-center pt-16 md:pt-24 p-4 bg-black/70 backdrop-blur-md animate-in fade-in duration-200">
      <div className="w-full max-w-2xl bg-[#FFFFFF] dark:bg-[#201A1A] rounded-3xl shadow-2xl border border-[#E2DBD0] dark:border-[#3B3030] overflow-hidden flex flex-col max-h-[80vh]">
        ...
        <div className="p-4 border-b border-[#E2DBD0] dark:border-[#3B3030] flex items-center gap-3 bg-[#FAF8F5] dark:bg-[#141010]/80">
          ...
          <input ref={inputRef} type="text" placeholder="Type a command, page name, or search transactions..." value={query} onChange={(e) => setQuery(e.target.value)} className="flex-1 bg-transparent text-sm md:text-base font-bold text-[#141010] dark:text-[#FAF8F5] focus:outline-none placeholder-[#594D4D]"/>
          {query && (<button onClick={() => setQuery("")} className="p-1 text-[#594D4D] hover:text-[#141010] dark:hover:text-[#FAF8F5]">
              <X className="w-4 h-4"/>
            </button>)}
          <button onClick={onClose} className="px-2 py-1 text-[10px] font-extrabold uppercase tracking-wider bg-[#E2DBD0]/60 dark:bg-[#3B3030] text-[#594D4D] dark:text-[#C8BFB0] rounded-lg">
            ESC
          </button>
        </div>
        ...
        <div className="flex-1 p-4 overflow-y-auto space-y-5 divide-y divide-[#E2DBD0]/40 dark:divide-[#3B3030]/40">
          ...
          {!query && (<div className="space-y-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                ...
                <button onClick={() => handleNavigate("/budgets")} className="flex items-center gap-3 p-3 rounded-2xl bg-[#FAF8F5] dark:bg-[#141010] hover:bg-[#810100]/10 border border-[#E2DBD0] dark:border-[#3B3030] text-left transition-all group">
          ...
          <div className="space-y-2 pt-3">
            ...
            const Icon = nav.icon;
            return (<button key={idx} onClick={() => handleNavigate(nav.path)} className="w-full flex items-center justify-between p-3 rounded-2xl hover:bg-[#FAF8F5] dark:hover:bg-[#141010] border border-transparent hover:border-[#E2DBD0] dark:hover:border-[#3B3030] text-left transition-all group">
          ...
          {query && (<div className="space-y-2 pt-3">
              <span className="text-[10px] font-extrabold uppercase tracking-wider text-[#594D4D] px-2 block">
                Matching Transactions ({filteredTx.length})
              </span>
              {filteredTx.length === 0 ? (<div className="p-4 text-center text-xs text-[#594D4D]">No matching transactions found</div>) : (<div className="space-y-1">
                  {filteredTx.slice(0, 5).map((tx) => (<div key={tx.id} onClick={() => handleNavigate("/transactions")} className="flex items-center justify-between p-3 rounded-2xl hover:bg-[#FAF8F5] dark:hover:bg-[#141010] border border-transparent hover:border-[#E2DBD0] dark:hover:border-[#3B3030] cursor-pointer transition-all">
                      <div>
                      ...
                      <span className={`text-xs font-black ${tx.type === "INCOME" ? "text-emerald-700 dark:text-emerald-400" : "text-[#810100] dark:text-[#FAF8F5]"}`}>
                        {tx.type === "INCOME" ? "+" : "-"}
                        {currency}
