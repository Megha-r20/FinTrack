...
export function StudentPacingCard({ summary, onApplyPreset, isApplyingPreset }) {
    const { user } = useAuth();
    ...
    return (<div className="bg-gradient-to-br from-[#FFFFFF] via-[#FAF8F5] to-[#F5EFE6] dark:from-[#201A1A] dark:via-[#1A1414] dark:to-[#141010] border border-[#E2DBD0] dark:border-[#3B3030] rounded-3xl p-6 shadow-md relative overflow-hidden space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#E2DBD0]/60 dark:border-[#3B3030]/60 pb-4">
        ...
        {onApplyPreset && (<button onClick={onApplyPreset} disabled={isApplyingPreset} className="flex items-center justify-center gap-2 px-3.5 py-2 rounded-xl text-xs font-extrabold bg-[#141010] dark:bg-[#FAF8F5] text-[#FAF8F5] dark:text-[#141010] hover:bg-black dark:hover:bg-white shadow-sm transition-all shrink-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-[#FFFFFF]/80 dark:bg-[#141010]/80 border border-[#E2DBD0] dark:border-[#3B3030] rounded-2xl p-4 space-y-1">
          ...
          <div className="w-full bg-[#FAF8F5] dark:bg-[#201A1A] h-2 rounded-full overflow-hidden border border-[#E2DBD0] dark:border-[#3B3030] mt-2">
            <div className={`h-full rounded-full transition-all duration-500 ${overallPercentage > 100
            ? "bg-rose-600"
            : overallPercentage >= 80
                ? "bg-amber-500"
                : "bg-gradient-to-r from-emerald-600 to-teal-500"}`} style={{ width: `${Math.min(overallPercentage, 100)}%` }}/>
          </div>
        </div>
      ...
      <div className="flex items-center gap-3 p-3.5 bg-[#FFFFFF]/90 dark:bg-[#141010]/90 border border-[#E2DBD0] dark:border-[#3B3030] rounded-2xl text-xs">
        {isOverpacing ? (<AlertTriangle className="w-4 h-4 text-amber-600 dark:text-amber-400 shrink-0"/>) : (<CheckCircle2 className="w-4 h-4 text-emerald-600 dark:text-emerald-400 shrink-0"/>)}
