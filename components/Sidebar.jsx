...
export function Sidebar({ isOpen, onClose }) {
    ...
    return (<aside className={`fixed top-0 left-0 z-40 h-screen w-64 border-r transition-all duration-300 flex flex-col justify-between bg-[#FAF8F5] dark:bg-[#141010] border-[#E2DBD0] dark:border-[#3B3030] ${isOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
      <div>
        {/* Brand Header */}
        ...
        <nav className="p-4 space-y-1.5 overflow-y-auto max-h-[calc(100vh-160px)]">
          {navItems.map((item) => {
            ...
            return (<Link key={item.href} href={item.href} onClick={onClose} className={`flex items-center justify-between px-4 py-3 rounded-2xl font-bold text-xs transition-all duration-200 ${isActive
                ...
                {item.badge && (<span className="flex items-center gap-1 text-[9px] font-extrabold px-2 py-0.5 rounded-full bg-[#810100]/10 dark:bg-[#810100]/25 text-[#810100] dark:text-[#FAF8F5] border border-[#810100]/30">
      ...
      <div className="p-4 border-t border-[#E2DBD0] dark:border-[#3B3030] bg-[#FFFFFF] dark:bg-[#201A1A]/90">
        ...
        <button onClick={logout} className="w-full flex items-center justify-center gap-2 px-3 py-2 text-xs font-extrabold text-[#810100] dark:text-[#FAF8F5] hover:bg-[#810100]/10 rounded-xl transition-colors border border-[#810100]/30">
